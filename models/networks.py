...
def convTranspose_norm_relu_module(norm_type, norm_layer, input_nc, ngf, kernel_size, padding, stride=1,
                                   output_padding=0):
...
def define_G_3d(input_nc, output_nc, norm='batch', groups=26, ksize=3, padding=1, gpu_ids=[]):
    ...
    netG_3d = ResnetGenerator_3d_conv(input_nc, output_nc, norm_type=norm, groups=groups, ksize=ksize, padding=padding,
                                      gpu_ids=gpu_ids)
...
def define_Enc(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[]):
    ...
    print(which_model_netG)
    if which_model_netG == 'resnet_9blocks':
        netG = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9,
                             norm_type=norm, gpu_ids=gpu_ids)
    elif which_model_netG == 'resnet_6blocks':
        netG = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6,
                             norm_type=norm, gpu_ids=gpu_ids)
    elif which_model_netG == 'unet_128':
...
def define_Dec(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[]):
    ...
    print(which_model_netG)
    if which_model_netG == 'resnet_9blocks':
        netG = ResnetDecoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9,
                             norm_type=norm, gpu_ids=gpu_ids)
    elif which_model_netG == 'resnet_6blocks':
        netG = ResnetDecoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6,
                             norm_type=norm, gpu_ids=gpu_ids)
    elif which_model_netG == 'unet_128':
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    if which_model_netD == 'basic':
        netD = NLayerDiscriminator(input_nc, ndf, n_layers=3, use_sigmoid=use_sigmoid, norm_layer=norm_layer,
                                   norm_type=norm, postConv=postConv, gpu_ids=gpu_ids)
    elif which_model_netD == 'n_layers':
        netD = NLayerDiscriminator(input_nc, ndf, n_layers_D, use_sigmoid=use_sigmoid, norm_layer=norm_layer,
                                   norm_type=norm, postConv=postConv, gpu_ids=gpu_ids)
    else:
...
class ResnetDecoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_downsampling):
            ...
            model += convTranspose_norm_relu_module(norm_type, norm_layer, ngf * mult, int(ngf * mult / factor_ch), 3,
                                                    1,
...
class ResnetEncoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_blocks):
            model += [
                ResnetBlock(ngf * mult, 'zero', norm_layer=norm_layer, use_dropout=use_dropout, norm_type=norm_type)]
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, norm_type='batch',
