    ...
    def all2observed(self, tensor_all):
        ...
        for batch in range(b):
            if not self.opt.rgb_in and self.opt.rgb_out:
                ...
                # TODO
                tensor_gt[batch, :, :, :] = tensor_all.data[batch, self.out_id[batch]*np.array(self.opt.input_nc_1):
    ...
    def optimize_parameters_Stacked(self, iter):
        ...
        for batch in range(self.opt.input_nc):
            if not self.opt.rgb_in and self.opt.rgb_out:
                ...
                # TODO
                fake_B0_grad[batch, self.obs[batch]*np.array(self.opt.input_nc_1):
                             (self.obs[batch]+1)*np.array(self.opt.input_nc_1), :, :] = real_A_grad[batch, :, :, :]
