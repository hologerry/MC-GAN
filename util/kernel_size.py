...
def convTranspose2d(input_dim, padding, kernel_size, stride, output_padding):
    output_dim = (input_dim-1)*stride - 2*padding + \
...
if __name__ == '__main__':
    ...
    if conv_type == 'downconv':
        ...
        print("kernel of dim (%s, %s) with stride %s, padding %s, " +
              "dilation %s has been applied on the input with dim (%s, %s)" % (
                kernel_size, kernel_size, stride, padding, dilation, input_dim, input_dim))
        ...
        if len(sys.argv) == 7:
            ...
            output_padding = 0
        output_dim = convTranspose2d(
            input_dim, padding, kernel_size, stride, output_padding)
        print("kernel of dim (%s, %s) with stride %s, padding %s, " +
              "dilation %s has been applied on the input with dim (%s, %s)" % (
                kernel_size, kernel_size, stride, padding, output_padding, input_dim, input_dim))
